...
var type:= 0 # 0: straight, 1: corner, 2: t-junction, 3: cross (oops never used)
var connected := false
...
func _on_area_2d_input_event(_viewport: Node, event: InputEvent, _shape_idx: int) -> void:
	if event is InputEventMouseButton and event.pressed and event.button_index == MOUSE_BUTTON_LEFT:
		pipe_rotation = (pipe_rotation + 1) % 4 # rotate pipe clockwise
		rotation = deg_to_rad(float(pipe_rotation) * 90.0) # rotate sprite
		sound.play()
